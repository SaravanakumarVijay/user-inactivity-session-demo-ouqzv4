...
export class UserIdleComponent implements OnInit {
  ...
  setCircleDasharray = (elapsedTime: number) => {
    ...
    const circleDasharray = `${(this.timeRemain * this.FULL_DASH_ARRAY).toFixed(
      0
    )} 283`;
    document
  ...
  initTimer(): void {
    ...
    this.ngIdle.USER_IDLE_TIMER_VALUE_IN_MIN = 1;
    this.ngIdle.FINAL_LEVEL_TIMER_VALUE_IN_MIN = 1;
    // end
